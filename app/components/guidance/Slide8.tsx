...
useEffect(() => {
  ...
  fetchData();
    ...
    <div className='container md:w-1/2 mx-auto m-3'>
        <h3 className='text-center'>SF Economy Express(International) Rates Policy</h3>
        ...
        <div className=' columns-1 text-center text-xl m-3'>
            ...
            {result && <div className=' text-amber-400 text-2xl'>{`Result: ${result} €`}</div>}
    ...
    <form onSubmit={handleSubmit} className=" m-3 container mx-auto space-y-6 rounded-md border-2 border-purple-600 bg-white p-4 text-purple-950 shadow-md">
 

        <label className='' htmlFor="shipmentType">Select shipment type:</label>
            <select id="shipmentType" value={shipmentType} onChange={handleShipmentType} className="block w-full rounded-md border-2 border-purple-400 p-2 focus:border-purple-500 disabled:border-purple-300 disabled:bg-purple-50"  required >
        ...
        <label className='' htmlFor="countrySelect">Select a Country:</label>
            <select id="countrySelect" value={selectedCountry} onChange={handleCountryChange} className="block w-full rounded-md border-2 border-purple-400 p-2 focus:border-purple-500 disabled:border-purple-300 disabled:bg-purple-50"  required >
        ...
        <div className="space-y-1">
            ...
            <input max={50} min={0.1} onChange={handleWeightChange} value={weight} type="number" step='0.1' maxLength={7} id="weight" className="block w-full rounded-md border-2 border-purple-400 p-2 focus:border-purple-500 disabled:border-purple-300 disabled:bg-purple-50" placeholder="Required Field" required />
        </div>
        <div className="space-y-1">
            <label htmlFor="length" className="block text-sm font-semibold">Length (cm)</label>
            <input min={1} max={120}  onChange={handleLengthChange} value={length} type="number" id="length" className="block w-full rounded-md border-2 border-purple-400 p-2 focus:border-purple-500 disabled:border-purple-300 disabled:bg-purple-50" placeholder="Required Field" required />
        </div>
        <div className="space-y-1">
            <label htmlFor="width" className="block text-sm font-semibold">Width (cm)</label>
            <input min={1} max={80}  onChange={handleWidthChange} value={width} type="number" id="width" className="block w-full rounded-md border-2 border-purple-400 p-2 focus:border-purple-500 disabled:border-purple-300 disabled:bg-purple-50" placeholder="Required Field" required />
        </div>
        <div className="space-y-1">
            <label htmlFor="height" className="block text-sm font-semibold">Height (cm)</label>
            <input  min={1} max={160} onChange={handleHeightChange} value={height} type="number" id="height" className="block w-full rounded-md border-2 border-purple-400 p-2 focus:border-purple-500 disabled:border-purple-300 disabled:bg-purple-50" placeholder="Required Field" required />
        </div>  



  <div className="space-x-2">
    <button className="rounded border-2 border-current bg-purple-300 px-2 py-1 text-purple-900 hover:bg-purple-400 active:bg-purple-500">Submit</button>
  </div>
</form>
</div>
